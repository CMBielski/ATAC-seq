...
bam_files$Condition <-
  rep(
    ...
    luq(bam_files$DFCI_ID)
  )

bam_files$Cell <-
  rep(paste0(c("CD4", "CD8"), "_memory"), nrow(bam_files) / 2)
...
colnames(cd4_counts) <- c("Pre-treatment", "Post-treatment")
colnames(cd8_counts) <- c("Pre-treatment", "Post-treatment")
...
plotMA(cd4_r_dds, alpha = 0.01, ylim = c(-3, 3))
dev.off()
...
plotMA(cd8_r_dds, alpha = 0.01, ylim = c(-3, 3))
dev.off()
...
cd4.mat <- zClust(cd4_count_mx_filtered[order(-rowVars)[1:1000], ], scale = "col")

side_cols <- rep(c("lightgrey", "dimgrey"), times = 3)

pdf("CD4_memory_clust.pdf", width = 14, height = 14, useDingbats = F)
heatmap.2(
  cd4.mat$data,
  trace = 'none',
...
pdf("CD8_Volcano.pdf", width = 14, height = 20)
...
cd8.mat <- zClust(cd8_count_mx_filtered[order(-rowVars)[1:1000], ], scale = "col")

side_cols <- rep(c("lightgrey", "dimgrey"), times = 3)

pdf("CD8_memory_clust.pdf", width = 14, height = 14, useDingbats = F)
heatmap.2(
  cd8.mat$data,
  trace = 'none',
  col = viridis(n=299, option = 'A'),
